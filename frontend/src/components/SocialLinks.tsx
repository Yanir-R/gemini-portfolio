...
interface SocialLink {
    href: string;
    label: string;
    /* Each service's own colour, applied on hover only: the icons sit in the
       header at rest, where three brand colours would compete with the one
       green the palette spends on state. */
    hoverColor: string;
    /* Outlined icons need `stroke`, filled ones need `fill`. */
    outlined?: boolean;
            ...
            {SOCIAL_LINKS.map((link) => (
                ...
                >
                    <svg
                        ...
                        fill={link.outlined ? 'none' : 'currentColor'}
                        stroke={link.outlined ? 'currentColor' : undefined}
                        viewBox="0 0 24 24"
                    >
                        {link.icon}
                    </svg>
