...
const NavLinks: React.FC<NavLinksProps> = ({ isMobile = false, className = "" }) => {
    ...
    const baseStyles = isMobile
        ? "block w-full px-4 py-3 text-base font-medium text-gray-300 transition-all duration-300 bg-dark-secondary hover:bg-[#2a2a2a] rounded-lg border border-transparent hover:border-border-hover"
    ...
    return (
        <div className={`${isMobile ? 'space-y-3 p-4' : 'flex space-x-8 text-sm font-medium'} ${className}`}>
            {links.map((link) => (
                <NavLink
                    key={link.href}
                    to={link.href}
